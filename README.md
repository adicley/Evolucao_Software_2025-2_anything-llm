...
```
1. Executar o script `extract_github_pr_comments.py` disponibilizado na pasta `script/` deste projeto.
2. Após isso, os comentários extraído ficarão salvos no arquivo `pr_comments.json` na mesma pasta que o script foi executado.
3. A partir disso, basta abrir o notebook `main.ipynb` disponibilizado no repositório e execultar as células em ordem para que seja gerado o arquivo de classificação e geração dos gráficos.
4. Ao final, os comentários classificados ficarão salvos no arquivo `pr_comments_classified.json` e os gráficos contendos os dados resumidos das classificações pelos modelos no próprio notebook disponibilizado.
